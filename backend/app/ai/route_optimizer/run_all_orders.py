...
from app.ai.route_optimizer.data_loader import DEFAULT_DATA_DIR, load_data
...
def main() -> None:
    ...
    orders = store.orders
    if not orders:
        orders_path = Path(DEFAULT_DATA_DIR) / "orders.csv"
    ...
    print(f"Baseline A_DIRECT_ROAD available orders: {baseline_a_available_count}/{len(orders)}")
