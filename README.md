...
(not a synthetic placeholder). Two components are backed by real data:
...
│   ├── llm_explainer.py    # Optional LLM-grounded explanation via the Anthropic API
│   ├── decision_engine.py  # Hard constraints + weighted priority scoring
│   ├── whatif.py           # Live scenario simulation for demo/Q&A
...
│   └── cache_shipments.py    # Precomputes the shipment sample used by the what-if endpoint
├── models/
...
│   └── ct_bound_shipments_cache.json    # Precomputed sample used by the what-if endpoint
├── tests/
...
The weights and threshold above were selected by
...
documented approximation, `reports/tuning_report.json` for the
...
falls back to the rolling-mean baseline automatically. `GET /api/v1/forecast` reports which one produced a given result
via `config.model_name` (`gradient_boosting_arrival_forecaster_v1` or `rolling_mean_v1`).

The model currently offers a modest improvement over the rolling-mean baseline
...
combined `flood_and_fleet_loss`), or a custom scenario can be constructed directly:

```bash
curl -X POST http://127.0.0.1:8001/api/v1/simulate/what-if \
  -H "Content-Type: application/json" \
  -d '{"preset": "flood"}'
```

The weather-delay scenario is a deliberate simplification: it scales the already-computed
hub-to-Cần-Thơ transit time for each historical shipment rather than re-running the full
...
(FastAPI, Pydantic, pandas, NumPy, scikit-learn, joblib, pytest, httpx, anthropic).
...
| `ANTHROPIC_API_KEY` | unset | Required only for [grounded explanations](#grounded-explanations); the feature is opt-in and fails open (returns `null`) if this is unset |
